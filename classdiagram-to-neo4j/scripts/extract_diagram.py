...
def extract_with_openai(
    ...
    model: str = "gpt-4o"  # Updated to current model
) -> Dict[str, Any]:
    """Extract diagram data using OpenAI Vision API with structured output."""
    if OpenAI is None:
    ...
    # Improved prompt with both-end cardinality and relationship semantics
    prompt = """Analyze this UML class diagram and extract ALL information with high accuracy:

1. ENTITIES (classes/boxes):
   - Full entity name
   - All properties (attributes) with:
     * Property name
     * Data type (string, integer, boolean, etc.)
     * Required flag (true/false)
     * Default value if shown
   - Visual indicators (color, border style)

2. RELATIONSHIPS (lines/arrows):
   - Source entity name
   - Target entity name
   - Relationship name/label (if shown on the line)
   - FROM cardinality (at source end: 0..1, 0..*, 1, 1..*, etc.)
   - TO cardinality (at target end: 0..1, 0..*, 1, 1..*, etc.)
   - Direction (out, in, bidirectional)
   - Role names (if shown at either end)
   - Relationship semantics:
     * Is it containment/composition? (filled diamond)
     * Is it aggregation? (empty diamond)
     * Is it inheritance? (triangle arrow)
     * Is the line dashed? (indicates separate diagram or optional)
   - Any relationship type labels

CRITICAL REQUIREMENTS:
- Extract BOTH cardinalities (fromCardinality and toCardinality) if visible
- If cardinality is NOT visible, use null (NOT a default value)
- Capture relationship semantics (containment, inheritance, etc.)
- Note if lines are dashed or solid
- Extract all role names at both ends of relationships

Return ONLY valid JSON (no markdown, no code blocks, no explanations)."""
    
    # Use structured output if available (OpenAI JSON mode)
    try:
        ...
        # Fallback to text extraction if JSON mode fails
        response = client.chat.completions.create(
            model=model,
            messages=[
                {
...
def extract_with_anthropic(
    image_path: str,
    api_key: Optional[str] = None,
    model: str = "claude-3-5-sonnet-20241022"  # Updated to current model
) -> Dict[str, Any]:
  ...
  "relationships": [
    {
      ...
      "fromCardinality": "0..1" or null,
      "toCardinality": "0..*" or null,
      "direction": "out",
      "role": "roleName" or null,
      "isContainment": true/false,
      "isInheritance": true/false,
      "isDashed": true/false
    }
  ],
  "metadata": {
...
def add_provenance(data: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    # Derive specId and diagramId from source path (matches populate_neo4j logic)
    path = Path(source)
    parts = path.parts
    ...
    # Check filename for spec prefix
    if not spec_id and "_" in diagram_id:
        parts = diagram_id.split("_", 1)
        if parts[0].startswith("tmf") or parts[0].startswith("spec"):
            spec_id = parts[0]
            diagram_id = parts[1] if len(parts) > 1 else diagram_id
    ...
    data["meta"]["specId"] = spec_id
    data["meta"]["diagramId"] = diagram_id
...
def extract_diagram(
    ...
    # Normalize relationship types and handle missing cardinalities
    if "relationships" in data:
        for rel in data["relationships"]:
            ...
            # Handle cardinality - don't default, use null if missing
            if "cardinality" in rel and not rel["cardinality"]:
                rel["cardinality"] = None
            
            # Ensure both cardinalities are present
            if "fromCardinality" not in rel:
                rel["fromCardinality"] = rel.get("cardinality")
            if "toCardinality" not in rel:
                rel["toCardinality"] = rel.get("cardinality")
